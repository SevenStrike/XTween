namespace SevenStrikeModules.XTween
{
    public partial class XTween_Controller
    {
        ...
        /// 动画 - XHud动画器方法
        /// </summary>
        private void TweenPlay_XHudElement()
        {
            if (TweenTypes != XTweenTypes.元素_XHudElement)
                return;

            if (TweenTypes_XHudElement == XTweenTypes_XHudElement.透明度_Opacity)
            {
                CurrentTweener = XTween.xt_Alpha_To(Target_HudElement, EndValue_Float, Duration, IsAutoKill, EaseMode, IsFromMode, () => FromValue_Float, UseCurve, Curve).SetLoop(LoopCount, LoopType).SetLoopingDelay(LoopDelay).SetDelay(Delay).OnStart(() =>
                {
