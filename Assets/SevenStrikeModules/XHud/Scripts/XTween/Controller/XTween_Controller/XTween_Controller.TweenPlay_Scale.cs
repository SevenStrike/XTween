namespace SevenStrikeModules.XTween
{
    ...
    public partial class XTween_Controller
    {
        ...
        /// 动画 - 缩放方法
        /// </summary>
        private void TweenPlay_Scale()
        {
            if (TweenTypes != XTweenTypes.缩放_Scale)
                return;

            CurrentTweener = XTween.xt_Scale_To(Target_RectTransform, EndValue_Vector3, Duration, IsRelative, IsAutoKill, EaseMode, IsFromMode, () => FromValue_Vector3, UseCurve, Curve).SetLoop(LoopCount, LoopType).SetLoopingDelay(LoopDelay).SetDelay(Delay).OnStart(() =>
            {
