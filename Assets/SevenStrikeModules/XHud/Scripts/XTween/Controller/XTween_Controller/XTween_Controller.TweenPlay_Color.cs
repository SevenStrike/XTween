namespace SevenStrikeModules.XTween
{
    ...
    public partial class XTween_Controller
    {
        ...
        /// 动画 - 颜色方法
        /// </summary>
        private void TweenPlay_Color()
        {
            if (TweenTypes != XTweenTypes.颜色_Color)
                return;

            CurrentTweener = XTween.xt_Color_To(Target_Image, EndValue_Color, Duration, IsAutoKill, EaseMode, IsFromMode, () => FromValue_Color, UseCurve, Curve).SetLoop(LoopCount, LoopType).SetLoopingDelay(LoopDelay).SetDelay(Delay).OnStart(() =>
            {
