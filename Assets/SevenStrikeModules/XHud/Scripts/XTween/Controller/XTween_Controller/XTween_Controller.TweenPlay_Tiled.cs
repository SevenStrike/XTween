namespace SevenStrikeModules.XTween
{
    public partial class XTween_Controller
    {
        ...
        /// 动画 - 平铺方法
        /// </summary>
        private void TweenPlay_Tiled()
        {
            if (TweenTypes != XTweenTypes.平铺_Tiled)
                return;

            CurrentTweener = XTween.xt_Tiled_To(Target_Image, EndValue_Float, Duration, IsAutoKill, EaseMode, IsFromMode, () => FromValue_Float, UseCurve, Curve).SetLoop(LoopCount, LoopType).SetLoopingDelay(LoopDelay).SetDelay(Delay).OnStart(() =>
            {
