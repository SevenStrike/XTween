namespace SevenStrikeModules.XTween
{
    ...
    public static class XTween_Previewer
    {
        ...
        private static void Initialize()
        {
            ...
            autoKillWithDuration = util_Tools.PlayerPrefs_ReadValue_Bool_ForEditor("PreviewTween_AutoKillWithDuration");
        ...
        public static void Append(XTween_Interface tween)
        {
            ...
            {
                util_Tools.Func_PrintInfo("XHud TweenPreview预览器消息", "动画预览器只有在非运行模式下才可使用！", HudMsgState.警告);
                return;
            }
        ...
        public static void Prepare(params XTween_Interface[] tweens)
        {
            ...
            if (AutoKillWithDuration && HasInfiniteLoopTween())
            {
                //util_Tools.Func_PrintInfo("XHud TweenPreview预览器消息", "检测到无限循环动画，自动停止功能将不会生效！", HudMsgState.警告);
            }
#endif
        }
        #endregion
        ...
        private static void CalculateMaxTotalDuration(bool Debug = false)
        {
            ...
            // 调试日志
            if (Debug)
                util_Tools.Func_PrintInfo("XHud TweenPreview预览器消息", $"(目前共有 {PreviewTweens.Count} 个动画，单个最大动画总时长：{MaxTotalDuration:F2}s", HudMsgState.警告);
        }
        /// <summary>
        /// 检测当前预览列表中是否存在无限循环动画
        /// 判断标准：
