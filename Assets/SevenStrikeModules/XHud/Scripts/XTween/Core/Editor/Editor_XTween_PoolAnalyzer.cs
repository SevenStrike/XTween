namespace SevenStrikeModules.XTween
{
    ...
    public class Editor_XTween_PoolAnalyzer : EditorWindow
    {
        ...
        {
            #region 图标获取
            logo = util_XHUDGUI.GetIcon("Icons_Hud_XTween_PoolAnalyzer/logo");
            icon_pathpercent = util_XHUDGUI.GetIcon("Icons_Hud_XTween_Controller/icon_pathpercent");
            LiquidBg_Ready = util_XHUDGUI.GetIcon("Icons_Liquid/XTween_PoolAnalyzer/LiquidBg_Ready");
            LiquidBg_Ready_Scanline = util_XHUDGUI.GetIcon("Icons_Liquid/XTween_PoolAnalyzer/LiquidBg_Ready_Scanline");
            LiquidBg_Playing = util_XHUDGUI.GetIcon("Icons_Liquid/XTween_PoolAnalyzer/LiquidBg_Playing");
            LiquidBg_Playing_Scanline = util_XHUDGUI.GetIcon("Icons_Liquid/XTween_PoolAnalyzer/LiquidBg_Playing_Scanline");
            LiquidPlug_Blue = util_XHUDGUI.GetIcon("Icons_Liquid/LiquidPlug_Blue");
            MetalGrid = util_XHUDGUI.GetIcon("Icons_Liquid/MetalGrid");
            LiquidDirty = util_XHUDGUI.GetIcon("Icons_Liquid/XTween_PoolAnalyzer/LiquidDirty");
            #endregion
        ...
        private void OnGUI()
        {
            ...
            util_XHUDGUI.Gui_Labelfield(Title_rect, "XTween 动画池管理器", HudFilled.无, HudColor.无, Color.white, TextAnchor.MiddleLeft, Vector2.zero, 20, Font_Bold);

            Sepline_rect = new Rect(rect.x + 85, rect.y + 60, 200, 1);
            util_XHUDGUI.Gui_Box(Sepline_rect, SepLineColor);

            util_XHUDGUI.Gui_Labelfield_Thin_WrapClip(new Rect(rect.x + 18, rect.y + 80, rect.width - 38, rect.height), "此面板可监控并管理XTween动画池的使用状态以及参数！", HudFilled.无, HudColor.无, MessageColor, TextAnchor.UpperLeft, new Vector2(0, 0), 12, true, Font_Light);
            #endregion
            ...
            util_XHUDGUI.Gui_LiquidField(rect_liquid_prim, TweenLiquidContent, liquid_rectoffet, TweenLiquidScreen);
            ...
            if (XTween_Dashboard.LiquidDirty)
            {
                ...
                util_XHUDGUI.Gui_TextureBox(rect_liquid_prim, LiquidDirty);
            }
            ...
            util_XHUDGUI.Gui_TextureBox(rect_liquid_prim, LiquidPlug_Blue);
            ...
            util_XHUDGUI.Gui_Labelfield_WrapText(rect_liquid_prim, $"状态 :  {(XTween_Pool.IsAnyTweenInUse() ? "正在使用" : "未使用")}", HudFilled.无, HudColor.无, Color.black, TextAnchor.MiddleRight, Vector2.zero, 11, false, false, TextClipping.Overflow, true, Font_Light);
            #endregion
            ...
            util_XHUDGUI.Gui_Layout_Space(590);
            util_XHUDGUI.Gui_Layout_Horizontal_Start(HudFilled.无, HudColor.无);
            util_XHUDGUI.Gui_Layout_Space(15);
            if (util_XHUDGUI.Gui_Layout_Button("回收所有动画 (快速强制)", "", HudFilled.实体, HudColor.深空灰, Color.white, 35, new RectOffset(), new Vector2(0, 0), TextAnchor.MiddleCenter, 12, Font_Light))
            {
                if (Application.isPlaying)
