namespace SevenStrikeModules.XTween
{
    ...
    public class Editor_XTween_Manager : Editor
    {
        ...
        private TweenDatas TwnData_Playing;
        private TweenDatas TwnData_Pausing;
        private TweenDatas TwnData_Comleted;
        private TweenDatas TwnData_HasLoop;
        ...
        private void OnEnable()
        {
            ...
            #region 图标获取
            icon_main = util_XHUDGUI.GetIcon("Icons_Hud_XTween_Manager/icon_main");
            icon_pathpercent = util_XHUDGUI.GetIcon("Icons_Hud_XTween_Controller/icon_pathpercent");
            LiquidBg_Ready = util_XHUDGUI.GetIcon("Icons_Liquid/XTween_Manager/LiquidBg_Ready");
            LiquidBg_Ready_Scanline = util_XHUDGUI.GetIcon("Icons_Liquid/XTween_Manager/LiquidBg_Ready_Scanline");
            LiquidBg_Playing = util_XHUDGUI.GetIcon("Icons_Liquid/XTween_Manager/LiquidBg_Playing");
            LiquidBg_Playing_Scanline = util_XHUDGUI.GetIcon("Icons_Liquid/XTween_Manager/LiquidBg_Playing_Scanline");
            LiquidBg_Status_Playing = util_XHUDGUI.GetIcon("Icons_Liquid/XTween_Manager/LiquidBg_Status_Playing");
            LiquidBg_Status_Playing_Scanline = util_XHUDGUI.GetIcon("Icons_Liquid/XTween_Manager/LiquidBg_Status_Playing_Scanline");
            LiquidBg_Status_Ready = util_XHUDGUI.GetIcon("Icons_Liquid/XTween_Manager/LiquidBg_Status_Ready");
            LiquidBg_Status_Ready_Scanline = util_XHUDGUI.GetIcon("Icons_Liquid/XTween_Manager/LiquidBg_Status_Ready_Scanline");
            icon_notrun = util_XHUDGUI.GetIcon("Icons_Liquid/XTween_Manager/icon_notrun");
            Font_Bold = util_XHUDGUI.GetFont("SS_Editor_Bold");
            Font_Light = util_XHUDGUI.GetFont("SS_Editor_Light");
            icon_preview_r = util_XHUDGUI.GetIcon("Icons_Hud_XTween_Manager/icon_preview_r");
            icon_preview_p = util_XHUDGUI.GetIcon("Icons_Hud_XTween_Manager/icon_preview_p");
            icon_rewind_r = util_XHUDGUI.GetIcon("Icons_Hud_XTween_Manager/icon_rewind_r");
            icon_rewind_p = util_XHUDGUI.GetIcon("Icons_Hud_XTween_Manager/icon_rewind_p");
            icon_recycle_r = util_XHUDGUI.GetIcon("Icons_Hud_XTween_Manager/icon_recycle_r");
            icon_recycle_p = util_XHUDGUI.GetIcon("Icons_Hud_XTween_Manager/icon_recycle_p");
            LiquidPlug_Green = util_XHUDGUI.GetIcon("Icons_Liquid/LiquidPlug_Green");
            LiquidPlug_Yellow = util_XHUDGUI.GetIcon("Icons_Liquid/LiquidPlug_Yellow");
            MetalGrid = util_XHUDGUI.GetIcon("Icons_Liquid/MetalGrid");
            LiquidDirty_Bg = util_XHUDGUI.GetIcon("Icons_Liquid/XTween_Manager/LiquidDirty_Bg");
            LiquidDirty_Bg_Small = util_XHUDGUI.GetIcon("Icons_Liquid/XTween_Manager/LiquidDirty_Bg_Small");
            LiquidDirty_Status = util_XHUDGUI.GetIcon("Icons_Liquid/XTween_Manager/LiquidDirty_Status");
            LiquidDirty_Status_Small = util_XHUDGUI.GetIcon("Icons_Liquid/XTween_Manager/LiquidDirty_Status_Small");
            ListIndex = util_XHUDGUI.GetIcon("Icons_Hud_XTween_Manager/ListIndex");
            ItemIcon = util_XHUDGUI.GetIcon("Icons_Hud_XTween_Manager/ItemIcon");
            Highlight = util_XHUDGUI.GetIcon("Icons_Hud_XTween_Manager/Highlight");
            #endregion

            liquid_rectoffet = new RectOffset(45, 45, 20, 20);

            #region 列表标题名称样式
            ListNameStyle = new GUIStyle(util_XHUDGUI.Style_LabelfieldBoldText);
        ...
        public override void OnInspectorGUI()
        {
            ...
            util_XHUDGUI.Gui_Layout_Banner(icon_main, HudFilled.实体, HudColor.深空灰, "XTween - 动画管理器", Color.white, 20, 20);
            util_XHUDGUI.Gui_Layout_Space(5);

            #region 动画批量操作
            util_XHUDGUI.Gui_Layout_Vertical_Start(HudFilled.纯色边框, HudColor.亮白, 5, "动画批量操作", XTween_Dashboard.Theme_Primary);
            ...
            if (util_XHUDGUI.Gui_Layout_Button(15, "全部播放", icon_preview_r, icon_preview_p))
            {
                if (Application.isPlaying)
                    BaseScript.Play_All();
                else
                {
                    util_Tools.Func_PrintInfo("XTween动画管理器消息", "应用未运行，只有在引用运行时期才可以使用此功能！", HudMsgState.警告);
                }
            ...
            #region 全部倒退
            GUI.enabled = true;
            if (util_XHUDGUI.Gui_Layout_Button(15, "全部倒退", icon_rewind_r, icon_rewind_p))
            {
                if (Application.isPlaying)
            ...
            #region 全部回收
            GUI.enabled = true;
            if (util_XHUDGUI.Gui_Layout_Button(15, "全部回收", icon_recycle_r, icon_recycle_p))
            {
                if (Application.isPlaying)
            ...
            #region 统计数据
            util_XHUDGUI.Gui_Layout_Vertical_Start(HudFilled.纯色边框, HudColor.亮白, 5, "统计数据", XTween_Dashboard.Theme_Primary);
            util_XHUDGUI.Gui_Layout_Space(5);
            ...
            util_XHUDGUI.Gui_LiquidField(rect_liquid_prim, TweenLiquidContent, liquid_rectoffet, TweenLiquidScreen_Status);
            ...
            if (!IsExtraExpandPanel)
            {
                if (XTween_Dashboard.LiquidDirty)
                {
                    ...
                    util_XHUDGUI.Gui_TextureBox(rect_liquid_prim, IsExpandPanelWidth ? LiquidDirty_Status : LiquidDirty_Status_Small);
                }
            }
            ...
            if (!IsExtraExpandPanel)
            {
                ...
                util_XHUDGUI.Gui_TextureBox(rect_liquid_prim, LiquidPlug_Green);
            }
            ...
            util_XHUDGUI.Gui_Labelfield(rect_liquid_prim, Application.isPlaying ? $"动画统计：{Count_Tweens} 个" : "", HudFilled.无, HudColor.无, Color.black, TextAnchor.MiddleRight, new Vector2(0, 0), 11, Font_Light);
            ...
            util_XHUDGUI.Gui_Layout_Space(254);
            util_XHUDGUI.Gui_Layout_Vertical_End();
            #endregion

            #region 活跃动画列表
            util_XHUDGUI.Gui_Layout_Vertical_Start(HudFilled.纯色边框, HudColor.亮白, 5, "活跃动画列表", XTween_Dashboard.Theme_Primary);
            util_XHUDGUI.Gui_Layout_Space(5);
            ...
            util_XHUDGUI.Gui_LiquidField(rect_liquid_prim, "", liquid_rectoffet, TweenLiquidScreen);
            ...
            if (!IsExtraExpandPanel)
            {
                ...
                util_XHUDGUI.Gui_TextureBox(rect_liquid_prim, LiquidPlug_Yellow);
            }
            ...
            if (!IsExtraExpandPanel)
            {
                if (XTween_Dashboard.LiquidDirty)
                {
                    ...
                    util_XHUDGUI.Gui_TextureBox(rect_liquid_prim, IsExpandPanelWidth ? LiquidDirty_Bg : LiquidDirty_Bg_Small);
                }
            }

            util_XHUDGUI.Gui_Layout_Space(0);
            util_XHUDGUI.Gui_Layout_Vertical_End();
            #endregion
        }
        ...
        private void ActiveTweensList(XTween_Manager manager)
        {
            ...
            if (activeTweens.Count == 0)
            {
                ...
                util_XHUDGUI.Gui_Layout_Space(180);
                util_XHUDGUI.Gui_Layout_Horizontal_Start(HudFilled.无, HudColor.无);
                util_XHUDGUI.Gui_Layout_FlexSpace();
                util_XHUDGUI.Gui_Layout_Icon(45, icon_notrun, new Vector2(0, -20));
                util_XHUDGUI.Gui_Layout_FlexSpace();
                util_XHUDGUI.Gui_Layout_Horizontal_End();
                util_XHUDGUI.Gui_Layout_Labelfield("当前没有活跃的动画", HudFilled.无, HudColor.无, Color.black * 0.8f, true, TextClipping.Ellipsis, TextAnchor.MiddleCenter, 12, Font_Light);
                util_XHUDGUI.Gui_Layout_Space(260);
            }
            else
            {
                util_XHUDGUI.Gui_Layout_Space(20);
        ...
        private void DrawTweenInfo(XTween_Interface tween)
        {
            ...
            Rect rect = util_XHUDGUI.Gui_GetLastRect();
            Rect rect_item = rect;
            util_XHUDGUI.Gui_Box(rect, Color.black * 0.25f);
            GUI.color = Color.black;
            rect_item.Set(rect.x + 8, rect.y + 25, 15, 15);
            util_XHUDGUI.Gui_TextureBox(rect_item, ListIndex);
            rect_item.Set(rect.x + 32, rect.y + 25, rect.width - 35, 15);
            util_XHUDGUI.Gui_Labelfield(rect_item, $"ID：{tween.UniqueId.ToString()}", HudFilled.无, HudColor.无, Color.white, TextAnchor.MiddleLeft, Vector2.zero, 11, true, TextClipping.Ellipsis, true, Font_Light);
            ...
            if (!IsExtraExpandPanel)
            {
                ...
                util_XHUDGUI.Gui_TextureBox(rect_item, EasePicBg);
                rect_item.Set(rect.width - 96, rect.y + 57, 100, 65);
                util_XHUDGUI.Gui_TextureBox(rect_item, GetEasePic(tween.EaseMode));

                if (tween.UseCustomEaseCurve)
                {
                    GUI.color = Color.black;
                    rect_item.Set(rect.width - 76, rect.y + 57, 100, 65);
                    util_XHUDGUI.Gui_Labelfield(rect_item, "CustomCurve", HudFilled.无, HudColor.无, Color.black, TextAnchor.MiddleLeft, 11, Font_Bold);
                }
            }
            #endregion
            ...
            util_XHUDGUI.Gui_Icon(rect_item, ItemIcon);
            rect_item.Set(rect.x + 30, rect.y + height - 2, Highlight.width, Highlight.height);
            util_XHUDGUI.Gui_Icon(rect_item, Highlight);
            rect_item.Set(rect.x + 38, rect.y + height, rect.width, 15);
            util_XHUDGUI.Gui_Labelfield(rect_item, "基础", HudFilled.无, HudColor.无, Color.black, TextAnchor.MiddleLeft, Vector2.zero, 13, true, TextClipping.Ellipsis, true, Font_Light);
            rect_item.Set(rect.x + 10, rect.y + height + 15, rect.width - 20, 1);
            util_XHUDGUI.Gui_Box(rect_item);
            rect_item.Set(rect.x + 10, rect.y + height + 28, (rect.width / 2) - 5, 15);
            util_XHUDGUI.Gui_Labelfield(rect_item, $"耗时：<b>{tween.ElapsedTime * 1000:F0}</b>ms / <b>{tween.Duration * 1000:F0}</b>ms", HudFilled.无, HudColor.无, Color.white, TextAnchor.MiddleLeft, Vector2.zero, 12, true, TextClipping.Ellipsis, true, Font_Light);
            rect_item.Set(rect.x + (rect.width / 2) + 10, rect.y + height + 28, (rect.width / 2) - 10, 15);
            util_XHUDGUI.Gui_Labelfield(rect_item, $"延迟：<b>{tween.Delay.ToString()}</b> s", HudFilled.无, HudColor.无, Color.white, TextAnchor.MiddleLeft, Vector2.zero, 12, true, TextClipping.Ellipsis, true, Font_Light);
            rect_item.Set(rect.x + 10, rect.y + height + 50, (rect.width / 2) - 5, 15);
            util_XHUDGUI.Gui_Labelfield(rect_item, $"缓动：<b>{tween.EaseMode.ToString()}</b>", HudFilled.无, HudColor.无, Color.white, TextAnchor.MiddleLeft, Vector2.zero, 12, true, TextClipping.Ellipsis, true, Font_Light);
            rect_item.Set(rect.x + (rect.width / 2) + 10, rect.y + height + 50, (rect.width / 2) - 10, 15);
            util_XHUDGUI.Gui_Labelfield(rect_item, $"曲线：<b>{(tween.UseCustomEaseCurve ? "使用" : "不使用")}</b>", HudFilled.无, HudColor.无, Color.white, TextAnchor.MiddleLeft, Vector2.zero, 12, true, TextClipping.Ellipsis, true, Font_Light);
            #endregion
            ...
            util_XHUDGUI.Gui_Icon(rect_item, Highlight);
            rect_item.Set(rect.x + 38, rect.y + height, rect.width, 15);
            util_XHUDGUI.Gui_Labelfield(rect_item, "循环", HudFilled.无, HudColor.无, Color.black, TextAnchor.MiddleLeft, Vector2.zero, 13, true, TextClipping.Ellipsis, true, Font_Light);
            rect_item.Set(rect.x + 10, rect.y + height + 15, rect.width - 20, 1);
            util_XHUDGUI.Gui_Box(rect_item);
            rect_item.Set(rect.x + 10, rect.y + height + 28, (rect.width / 2) - 5, 15);
            util_XHUDGUI.Gui_Labelfield(rect_item, $"次数：<b>{tween.LoopCount.ToString()}</b> s", HudFilled.无, HudColor.无, Color.white, TextAnchor.MiddleLeft, Vector2.zero, 12, true, TextClipping.Ellipsis, true, Font_Light);
            rect_item.Set(rect.x + (rect.width / 2) + 10, rect.y + height + 28, (rect.width / 2) - 10, 15);
            util_XHUDGUI.Gui_Labelfield(rect_item, $"递归：<b>{tween.CurrentLoop.ToString()}</b> 次", HudFilled.无, HudColor.无, Color.white, TextAnchor.MiddleLeft, Vector2.zero, 12, true, TextClipping.Ellipsis, true, Font_Light);
            rect_item.Set(rect.x + 10, rect.y + height + 50, (rect.width / 2) - 5, 15);
            util_XHUDGUI.Gui_Labelfield(rect_item, $"模式：<b>{tween.LoopType.ToString()}</b>", HudFilled.无, HudColor.无, Color.white, TextAnchor.MiddleLeft, Vector2.zero, 12, true, TextClipping.Ellipsis, true, Font_Light);
            rect_item.Set(rect.x + (rect.width / 2) + 10, rect.y + height + 50, (rect.width / 2) - 10, 15);
            util_XHUDGUI.Gui_Labelfield(rect_item, $"间隔：<b>{tween.LoopingDelay.ToString()}</b> s", HudFilled.无, HudColor.无, Color.white, TextAnchor.MiddleLeft, Vector2.zero, 12, true, TextClipping.Ellipsis, true, Font_Light);
            #endregion
            ...
            util_XHUDGUI.Gui_Icon(rect_item, Highlight);
            rect_item.Set(rect.x + 38, rect.y + height, rect.width, 15);
            util_XHUDGUI.Gui_Labelfield(rect_item, "状态", HudFilled.无, HudColor.无, Color.black, TextAnchor.MiddleLeft, Vector2.zero, 13, true, TextClipping.Ellipsis, true, Font_Light);
            rect_item.Set(rect.x + 72, rect.y + height, rect.width, 15);
            util_XHUDGUI.Gui_Labelfield(rect_item, $"({GetStateString(tween)})", HudFilled.无, HudColor.无, Color.black, TextAnchor.MiddleLeft, Vector2.zero, 11, true, TextClipping.Ellipsis, true, Font_Light);
            rect_item.Set(rect.x + 10, rect.y + height + 15, rect.width - 20, 1);
            util_XHUDGUI.Gui_Box(rect_item);
            rect_item.Set(rect.x + 10, rect.y + height + 28, (rect.width / 2) - 5, 15);
            util_XHUDGUI.Gui_Labelfield(rect_item, $"自动杀死：<b>{tween.AutoKill.ToString()}</b>", HudFilled.无, HudColor.无, Color.white, TextAnchor.MiddleLeft, Vector2.zero, 12, true, TextClipping.Ellipsis, true, Font_Light);
            rect_item.Set(rect.x + (rect.width / 2) + 10, rect.y + height + 28, (rect.width / 2) - 10, 15);
            util_XHUDGUI.Gui_Labelfield(rect_item, $"相对模式：<b>{tween.IsRelative.ToString()}</b>", HudFilled.无, HudColor.无, Color.white, TextAnchor.MiddleLeft, Vector2.zero, 12, true, TextClipping.Ellipsis, true, Font_Light);
            rect_item.Set(rect.x + 10, rect.y + height + 50, (rect.width / 2) - 5, 15);
            util_XHUDGUI.Gui_Labelfield(rect_item, $"暂停状态：<b>{tween.IsPaused.ToString()}</b>", HudFilled.无, HudColor.无, Color.white, TextAnchor.MiddleLeft, Vector2.zero, 12, true, TextClipping.Ellipsis, true, Font_Light);
            rect_item.Set(rect.x + (rect.width / 2) + 10, rect.y + height + 50, (rect.width / 2) - 10, 15);
            util_XHUDGUI.Gui_Labelfield(rect_item, $"完成状态：<b>{tween.IsCompleted.ToString()}</b>", HudFilled.无, HudColor.无, Color.white, TextAnchor.MiddleLeft, Vector2.zero, 12, true, TextClipping.Ellipsis, true, Font_Light);
            #endregion

            util_XHUDGUI.Gui_Layout_Space(370);
            GUI.color = Color.white;
        }
        ...
        private void LiquidProgress(Rect rect_progress, float height, float mar_left, float mar_right, float mar_value, string title, string value, float progress)
        {
            ...
            util_XHUDGUI.Gui_Labelfield(r_pro, title, HudFilled.无, HudColor.无, Color.black * 0.9f, TextAnchor.MiddleLeft, new Vector2(0, 0), 12, Font_Light);
            // 数值
            r_pro.Set(rect_progress.x + (rect_progress.width - mar_right - mar_value), rect_progress.y + (height - 17), 50, 6);
            util_XHUDGUI.Gui_Labelfield(r_pro, value, HudFilled.无, HudColor.无, Color.black * 0.95f, TextAnchor.MiddleRight, new Vector2(0, 0), 11, Font_Light);
            // 起点线
            ...
            util_XHUDGUI.Gui_Icon(r_pro, icon_pathpercent);
            // 中点线
        ...
        {
            return AssetDatabase.LoadAssetAtPath<Texture2D>($"{XTween_Dashboard.Get_GUIStyle_Path()}Icon/EaseCurveGraph/{ease}.png");
        }
        /// <summary>
        /// 获取缓动参数曲线图背景
        /// </summary>        
