    ...
    public static partial class XTween
    {
        ...
        public static XTween_Interface xt_Rotate_To(this UnityEngine.RectTransform rectTransform, Vector3 endValue, float duration, bool isRelative = false, bool autokill = false)
        {
            ...
            if (Application.isPlaying)
            {
                ...
                tweener.Initialize(currentRotation, targetRotation, duration * XTween_Dashboard.HudManagerGet().DurationMultiply);
            ...
            else
            {
                ...
                tweener = new XTween_Specialized_Vector3(currentRotation, targetRotation, duration * XTween_Dashboard.HudManagerGet().DurationMultiply)
        ...
        public static XTween_Interface xt_Rotate_To(this UnityEngine.RectTransform rectTransform, Vector3 endValue, float duration, bool isRelative = false, bool autokill = false, EaseMode easeMode = EaseMode.InOutCubic, bool isFromMode = true, XTween_Getter<Vector3> fromvalue = null, bool useCurve = false, AnimationCurve curve = null)
        {
            ...
            else
            {
                ...
                if (isFromMode)
                {
                    ...
                    if (useCurve)// 使用曲线
                    {
                        tweener = new XTween_Specialized_Vector3(currentRotation, targetRotation, duration * XTween_Dashboard.HudManagerGet().DurationMultiply).OnUpdate((rotation, linearProgress, time) => { if (isRelative) { rectTransform.localEulerAngles = rotation; } else { rectTransform.eulerAngles = rotation; } }).OnRewind(() => { if (isRelative) { rectTransform.localEulerAngles = currentRotation; } else { rectTransform.eulerAngles = currentRotation; } }).OnComplete((duration) => { if (isRelative) { rectTransform.localEulerAngles = targetRotation; } else { rectTransform.eulerAngles = targetRotation; } }).SetFrom(fromval).SetEase(curve).SetAutokill(false).SetRelative(isRelative);
                    }
                    else
                    {
                ...
                else
                {
                    if (useCurve)// 使用曲线
                    {
                        tweener = new XTween_Specialized_Vector3(currentRotation, targetRotation, duration * XTween_Dashboard.HudManagerGet().DurationMultiply).OnUpdate((rotation, linearProgress, time) => { if (isRelative) { rectTransform.localEulerAngles = rotation; } else { rectTransform.eulerAngles = rotation; } }).OnRewind(() => { if (isRelative) { rectTransform.localEulerAngles = currentRotation; } else { rectTransform.eulerAngles = currentRotation; } }).OnComplete((duration) => { if (isRelative) { rectTransform.localEulerAngles = targetRotation; } else { rectTransform.eulerAngles = targetRotation; } }).SetEase(curve).SetAutokill(false).SetRelative(isRelative);
                    }
                    else
                    {
                        tweener = new XTween_Specialized_Vector3(currentRotation, targetRotation, duration * XTween_Dashboard.HudManagerGet().DurationMultiply).OnUpdate((rotation, linearProgress, time) => { if (isRelative) { rectTransform.localEulerAngles = rotation; } else { rectTransform.eulerAngles = rotation; } }).OnRewind(() => { if (isRelative) { rectTransform.localEulerAngles = currentRotation; } else { rectTransform.eulerAngles = currentRotation; } }).OnComplete((duration) => { if (isRelative) { rectTransform.localEulerAngles = targetRotation; } else { rectTransform.eulerAngles = targetRotation; } }).SetEase(easeMode).SetAutokill(false).SetRelative(isRelative);
                    }
                }
                return tweener;
            }
        ...
        public static XTween_Interface xt_Rotate_To(this UnityEngine.RectTransform rectTransform, Quaternion endValue, float duration, bool isRelative = false, bool autokill = false, HudRotateMode mode = HudRotateMode.SlerpUnclamped)
        {
            ...
            {
                XTween_Interface tweener;
                tweener = new XTween_Specialized_Quaternion(currentRotation, targetRotation, duration * XTween_Dashboard.HudManagerGet().DurationMultiply, mode)
                     .OnUpdate((rotation, linearProgress, time) =>
                     {
                         if (isRelative)
                         {
        ...
        public static XTween_Interface xt_Rotate_To(this UnityEngine.RectTransform rectTransform, Quaternion endValue, float duration, bool isRelative = false, bool autokill = false, HudRotateMode mode = HudRotateMode.SlerpUnclamped, EaseMode easeMode = EaseMode.InOutCubic, bool isFromMode = true, XTween_Getter<Quaternion> fromvalue = null, bool useCurve = false, AnimationCurve curve = null)
        {
            ...
            else
            {
                ...
                if (isFromMode)
                {
                    ...
                    if (useCurve)// 使用曲线
                    {
                        tweener = new XTween_Specialized_Quaternion(currentRotation, targetRotation, duration * XTween_Dashboard.HudManagerGet().DurationMultiply, mode).OnUpdate((rotation, linearProgress, time) => { if (isRelative) { rectTransform.localRotation = rotation; } else { rectTransform.rotation = rotation; } }).OnRewind(() => { if (isRelative) { rectTransform.localRotation = currentRotation; } else { rectTransform.rotation = currentRotation; } }).OnComplete((duration) => { if (isRelative) { rectTransform.localRotation = targetRotation; } else { rectTransform.rotation = targetRotation; } }).SetFrom(fromval).SetEase(curve).SetAutokill(false).SetRelative(isRelative);
                    }
                    else
                    {
                        tweener = new XTween_Specialized_Quaternion(currentRotation, targetRotation, duration * XTween_Dashboard.HudManagerGet().DurationMultiply, mode).OnUpdate((rotation, linearProgress, time) => { if (isRelative) { rectTransform.localRotation = rotation; } else { rectTransform.rotation = rotation; } }).OnRewind(() => { if (isRelative) { rectTransform.localRotation = currentRotation; } else { rectTransform.rotation = currentRotation; } }).OnComplete((duration) => { if (isRelative) { rectTransform.localRotation = targetRotation; } else { rectTransform.rotation = targetRotation; } }).SetFrom(fromval).SetEase(easeMode).SetAutokill(false).SetRelative(isRelative);
