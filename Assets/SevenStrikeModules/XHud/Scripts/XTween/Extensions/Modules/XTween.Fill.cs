        ...
        public static XTween_Interface xt_Fill_To(this Image image, float endValue, float duration, bool autokill = false, EaseMode easeMode = EaseMode.InOutCubic, bool isFromMode = true, XTween_Getter<float> fromvalue = null, bool useCurve = false, AnimationCurve curve = null)
        {
            ...
            else
            {
                ...
                if (isFromMode)
                {
                    ...
                    if (useCurve)// 使用曲线
                    {
                        tweener = new XTween_Specialized_Float(startalpha, endValue, duration * XTween_Dashboard.HudManagerGet().DurationMultiply).OnUpdate((fill, linearProgres, time) => { image.fillAmount = fill; }).OnRewind(() => { image.fillAmount = startalpha; }).OnComplete((duration) => { image.fillAmount = endValue; }).SetFrom(fromval).SetEase(curve).SetAutokill(false);
                    }
                    else
                    {
                        tweener = new XTween_Specialized_Float(startalpha, endValue, duration * XTween_Dashboard.HudManagerGet().DurationMultiply).OnUpdate((fill, linearProgres, time) => { image.fillAmount = fill; }).OnRewind(() => { image.fillAmount = startalpha; }).OnComplete((duration) => { image.fillAmount = endValue; }).SetFrom(fromval).SetEase(easeMode).SetAutokill(false);
                    }
                }
                else
                {
                    if (useCurve)// 使用曲线
                    {
                        tweener = new XTween_Specialized_Float(startalpha, endValue, duration * XTween_Dashboard.HudManagerGet().DurationMultiply).OnUpdate((fill, linearProgres, time) => { image.fillAmount = fill; }).OnRewind(() => { image.fillAmount = startalpha; }).OnComplete((duration) => { image.fillAmount = endValue; }).SetEase(curve).SetAutokill(false);
                    }
                    else
                    {
                        tweener = new XTween_Specialized_Float(startalpha, endValue, duration * XTween_Dashboard.HudManagerGet().DurationMultiply).OnUpdate((fill, linearProgres, time) => { image.fillAmount = fill; }).OnRewind(() => { image.fillAmount = startalpha; }).OnComplete((duration) => { image.fillAmount = endValue; }).SetEase(easeMode).SetAutokill(false);
                    }
                }
