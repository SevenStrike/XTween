    ...
    public static partial class XTween
    {
        ...
        public static XTween_Interface To(XTween_Getter<Color> getter, XTween_Setter<Color> setter, Color endValue, float duration, bool autokill = true)
        {
            ...
            else
            {
                ...
                tweener = new XTween_Specialized_Color(startValue, endValue, duration * XTween_Dashboard.HudManagerGet().DurationMultiply)
        ...
        public static XTween_Interface To(XTween_Getter<Color> getter, XTween_Setter<Color> setter, Color endValue, float duration, bool autokill = true, EaseMode easeMode = EaseMode.InOutCubic, bool isFromMode = true, XTween_Getter<Color> fromvalue = null, bool useCurve = false, AnimationCurve curve = null)
        {
            ...
            else
            {
                ...
                if (isFromMode)
                {
                    ...
                    if (useCurve)// 使用曲线
                    {
                        tweener = new XTween_Specialized_Color(startValue, endValue, duration * XTween_Dashboard.HudManagerGet().DurationMultiply).OnUpdate((value, linearProgress, time) => { setter(value); }).OnRewind(() => { setter(startValue); }).SetFrom(fromval).SetEase(curve).SetAutokill(false);
                    }
                    else
                    {
                        tweener = new XTween_Specialized_Color(startValue, endValue, duration * XTween_Dashboard.HudManagerGet().DurationMultiply).OnUpdate((value, linearProgress, time) => { setter(value); }).OnRewind(() => { setter(startValue); }).SetFrom(fromval).SetEase(easeMode).SetAutokill(false);
                    }
                }
                else
                {
                    if (useCurve)// 使用曲线
                    {
                        tweener = new XTween_Specialized_Color(startValue, endValue, duration * XTween_Dashboard.HudManagerGet().DurationMultiply).OnUpdate((value, linearProgress, time) => { setter(value); }).OnRewind(() => { setter(startValue); }).SetEase(curve).SetAutokill(false);
                    }
                    else
                    {
                        tweener = new XTween_Specialized_Color(startValue, endValue, duration * XTween_Dashboard.HudManagerGet().DurationMultiply).OnUpdate((value, linearProgress, time) => { setter(value); }).OnRewind(() => { setter(startValue); }).SetEase(easeMode).SetAutokill(false);
                    }
                }
                return tweener;
            }
