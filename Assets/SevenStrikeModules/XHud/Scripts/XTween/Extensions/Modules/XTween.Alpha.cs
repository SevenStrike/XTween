    ...
    public static partial class XTween
    {
        ...
        public static XTween_Interface xt_Alpha_To(this Image image, float endValue, float duration, bool autokill = false)
        {
            ...
            if (Application.isPlaying)
            {
                ...
                tweener.Initialize(startColor.a, endValue, duration * XTween_Dashboard.HudManagerGet().DurationMultiply);
            ...
            else
            {
                ...
                tweener = new XTween_Specialized_Float(startColor.a, endValue, duration * XTween_Dashboard.HudManagerGet().DurationMultiply)
        ...
        public static XTween_Interface xt_Alpha_To(this CanvasGroup canvasgroup, float endValue, float duration, bool autokill = false, EaseMode easeMode = EaseMode.InOutCubic, bool isFromMode = true, XTween_Getter<float> fromvalue = null, bool useCurve = false, AnimationCurve curve = null)
        {
            ...
            else
            {
                ...
                if (isFromMode)
                {
                    ...
                    if (useCurve)// 使用曲线
                    {
                        tweener = new XTween_Specialized_Float(startalpha, endValue, duration * XTween_Dashboard.HudManagerGet().DurationMultiply).OnUpdate((alpha, linearProgres, time) => { canvasgroup.alpha = alpha; }).OnRewind(() => { canvasgroup.alpha = startalpha; }).OnComplete((duration) => { canvasgroup.alpha = endValue; }).SetFrom(fromval).SetEase(curve).SetAutokill(false);
                    }
                    else
                    {
                        tweener = new XTween_Specialized_Float(startalpha, endValue, duration * XTween_Dashboard.HudManagerGet().DurationMultiply).OnUpdate((alpha, linearProgres, time) => { canvasgroup.alpha = alpha; }).OnRewind(() => { canvasgroup.alpha = startalpha; }).OnComplete((duration) => { canvasgroup.alpha = endValue; }).SetFrom(fromval).SetEase(easeMode).SetAutokill(false);
                ...
                else
                {
                    if (useCurve)// 使用曲线
                    {
                        tweener = new XTween_Specialized_Float(startalpha, endValue, duration * XTween_Dashboard.HudManagerGet().DurationMultiply).OnUpdate((alpha, linearProgres, time) => { canvasgroup.alpha = alpha; }).OnRewind(() => { canvasgroup.alpha = startalpha; }).OnComplete((duration) => { canvasgroup.alpha = endValue; }).SetEase(curve).SetAutokill(false);
                    }
                    else
                    {
                        tweener = new XTween_Specialized_Float(startalpha, endValue, duration * XTween_Dashboard.HudManagerGet().DurationMultiply).OnUpdate((alpha, linearProgres, time) => { canvasgroup.alpha = alpha; }).OnRewind(() => { canvasgroup.alpha = startalpha; }).OnComplete((duration) => { canvasgroup.alpha = endValue; }).SetEase(easeMode).SetAutokill(false);
                    }
                }
