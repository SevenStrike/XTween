        ...
        public static XTween_Interface xt_Color_To(this Image image, Color endValue, float duration, bool autokill = false, EaseMode easeMode = EaseMode.InOutCubic, bool isFromMode = true, XTween_Getter<Color> fromvalue = null, bool useCurve = false, AnimationCurve curve = null)
        {
            ...
            else
            {
                ...
                if (isFromMode)
                {
                    ...
                    if (useCurve)// 使用曲线
                    {
                        tweener = new XTween_Specialized_Color(startColor, endValue, duration * XTween_Dashboard.HudManagerGet().DurationMultiply).OnUpdate((color, linearProgress, time) => { image.color = color; }).OnRewind(() => { image.color = startColor; }).OnComplete((duration) => { image.color = endValue; }).SetFrom(fromval).SetEase(curve).SetAutokill(false);
                    }
                    else
                    {
                        tweener = new XTween_Specialized_Color(startColor, endValue, duration * XTween_Dashboard.HudManagerGet().DurationMultiply).OnUpdate((color, linearProgress, time) => { image.color = color; }).OnRewind(() => { image.color = startColor; }).OnComplete((duration) => { image.color = endValue; }).SetFrom(fromval).SetEase(easeMode).SetAutokill(false);
                    }
                }
                else
                {
                    if (useCurve)// 使用曲线
                    {
                        tweener = new XTween_Specialized_Color(startColor, endValue, duration * XTween_Dashboard.HudManagerGet().DurationMultiply).OnUpdate((color, linearProgress, time) => { image.color = color; }).OnRewind(() => { image.color = startColor; }).OnComplete((duration) => { image.color = endValue; }).SetEase(curve).SetAutokill(false);
                    }
                    else
                    {
                        tweener = new XTween_Specialized_Color(startColor, endValue, duration * XTween_Dashboard.HudManagerGet().DurationMultiply).OnUpdate((color, linearProgress, time) => { image.color = color; }).OnRewind(() => { image.color = startColor; }).OnComplete((duration) => { image.color = endValue; }).SetEase(easeMode).SetAutokill(false);
                    }
                }
                return tweener;
            }
