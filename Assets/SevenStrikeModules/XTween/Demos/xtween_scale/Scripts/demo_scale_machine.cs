...
public class demo_scale_machine : demo_base
{
    ...
    public override void Tween_Create()
    {
        ...
        if (dir == "display")
        {
            ...
            rulertween.tween = rulertween.img.rectTransform.xt_Rotate_To(rulertween.target, rulertween.duration, false, true, XTweenRotationSpace.相对, XTweenRotationMode.Shortest, Ruler_easeMode, true, () => Vector3.forward * 210, false, null).SetDelay(Ruler_Delay).SetLoop(0);
    ...
    {
        twn.tween = twn.img.rectTransform.xt_Rotate_To(twn.target, twn.duration, false, false, XTweenRotationSpace.相对, XTweenRotationMode.FullRotation).OnKill(() =>
        {
            if (twn.img != null)
                twn.img.rectTransform.eulerAngles = Vector3.zero;
