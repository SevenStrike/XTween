    ...
    public static partial class XTween
    {
        ...
        public static XTween_Interface To(XTween_Getter<float> getter, XTween_Setter<float> setter, float endValue, float duration, bool autokill = true)
        {
            ...
            else
            {
                ...
                tweener = new XTween_Specialized_Float(startValue, endValue, duration * XTween_Dashboard.DurationMultiply)
        ...
        public static XTween_Interface To(XTween_Getter<float> getter, XTween_Setter<float> setter, float endValue, float duration, bool autokill = true, EaseMode easeMode = EaseMode.InOutCubic, bool isFromMode = true, XTween_Getter<float> fromvalue = null, bool useCurve = false, AnimationCurve curve = null)
        {
            ...
            else
            {
                ...
                if (isFromMode)
                {
                    ...
                    if (useCurve)// 使用曲线
                    {
                        tweener = new XTween_Specialized_Float(startValue, endValue, duration * XTween_Dashboard.DurationMultiply).OnUpdate((value, linearProgress, time) => { setter(value); }).OnRewind(() => { setter(startValue); }).SetEase(curve).SetFrom(fromval).SetAutokill(false);
                    }
                    else// 使用缓动参数
                    {
                        tweener = new XTween_Specialized_Float(startValue, endValue, duration * XTween_Dashboard.DurationMultiply).OnUpdate((value, linearProgress, time) => { setter(value); }).OnRewind(() => { setter(startValue); }).SetEase(easeMode).SetFrom(fromval).SetAutokill(false);
                    }
                }
                // 从当前值开始
                else
                {
                    if (useCurve)// 使用曲线
                    {
                        tweener = new XTween_Specialized_Float(startValue, endValue, duration * XTween_Dashboard.DurationMultiply).OnUpdate((value, linearProgress, time) => { setter(value); }).OnRewind(() => { setter(startValue); }).SetEase(curve).SetAutokill(false);
                    }
                    else// 使用缓动参数
                    {
                        tweener = new XTween_Specialized_Float(startValue, endValue, duration * XTween_Dashboard.DurationMultiply).OnUpdate((value, linearProgress, time) => { setter(value); }).OnRewind(() => { setter(startValue); }).SetEase(easeMode).SetAutokill(false);
                    }
                }
                return tweener;
            }
